...
<script type="text/javascript">
    ...
    var queryStringKeyAsyncOrder = "<%= BrightcoveVideoCloudIntegration.VideoCloudWebPart.QueryStringKeyAsyncOrder %>";
    var queryStringKeyAsyncSort = "<%= BrightcoveVideoCloudIntegration.VideoCloudWebPart.QueryStringKeyAsyncSort %>";
    ...
    function DoFilter(elemInput, type) {
        var order;
        var sort;
        try {
            if (type == 'order') {
                order = elemInput.value;
                sort = $('input[name=sort]:radio:checked').val();
            }
            else {
                order = $('input[name=order]:radio:checked').val();                
                sort = elemInput.value;
            }

            var searchText = $('.searchBox input').val();
            
            if(searchText == 'search video') {
                searchText = '';
            }                                                         
    ...
    function SelectToggleItem(parentElem) {
        $('div.videosAvailable div.item').removeClass('videoSelected');
        ...
        if ($('div.videosAvailable div.videoSelected').length > 0) {
    ...
    function DoSearch(elemInput) {
        try {
            ...
            var order = $('input[name=order]:radio:checked').val();
            var sort = $('input[name=sort]:radio:checked').val();
...
<div id="divPicklist" style="display:none;">
    <div class="picklist available">
        ...
        <div>
            Sort by: 
            <input type="radio" name="sort" onclick="DoFilter(this,'sort')" value="relevance" checked /><span>relevance</span>
            <input type="radio" name="sort" onclick="DoFilter(this, 'sort')" value="display name" /><span>display name</span>
            <input type="radio" name="sort" onclick="DoFilter(this, 'sort')" value="creation date" /><span>creation date</span>
            <input type="radio" name="sort" onclick="DoFilter(this, 'sort')" value="start date" /><span>start date</span>
        </div>
        <div>
            Order by: 
