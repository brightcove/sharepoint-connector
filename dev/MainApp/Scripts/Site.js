...
$(document).ready(function () {
    SP.SOD.loadMultiple(['sp.js', 'core.js'], customCode);
});
...
BrightCove.BCApp.ViewModels = {
    ...
    AddVideosViewModel: function () {
        ...
        var GetImageFileBuffer = function (prependText, elementId) {
            var dfd = $.Deferred();
            if ($(elementId)[0].files != null && $(elementId)[0].files.length > 0) {
        ...
        me.SearchBtnClick = function (ViewData, Event) {
            ...
            if (searchScope == 'VideoCloud') {
                ...
                if (splitTerms != null && splitTerms.length > 0) {
                    ...
                    tagSearchText = splitTerms.join('&any=tag:');
                }
                ...
                if (searchText != '' && searchText.length > 1) {
                    url = "https://api.brightcove.com/services/library?command=search_videos"
                        + "&any=" + searchText + "&any=tag:" + tagSearchText
                ...
                if (me.FullVideoListData().length == 0)
                {
    ...
    ManagePlaylistsViewModel: function () {
        ...
        $('#ddlSelectAccount').change(function () {
            ...
            $('#sorted').html('').hide();
                ...
                var filteredItems = ko.utils.arrayFilter(vData, function (item) {
                    return item.Account == currentAccount;
                });
                me.FilteredSPVideos(filteredItems);
                // me.FilteredSPVideos(ko.mapping.fromJS(vData)())
    ...
    CheckLists: function () {
        ...
        BrightCove.BCApp.Installer.Get_isListExists(BrightCove.BCApp.Constants.SharePointConstants.SPListID_Accounts,
                function (listExists) {
                    if (listExists == false) {
                        ...
                        var btnClass = $("#btnAccounts").attr('class').replace('hidden', '');
                        $("#btnAccounts").attr('class', btnClass);
                    }
                }, function (sender, args) {}
            );
        // Videos List
        BrightCove.BCApp.Installer.Get_isListExists(BrightCove.BCApp.Constants.SharePointConstants.SPListID_VideoList,
                function (listExists) {
                    if (listExists == false) {
                        BrightCove.BCApp.Installer.HideNavigationButtons();
                        var btnClass = $("#btnVideos").attr('class').replace('hidden', '');
                        $("#btnVideos").attr('class', btnClass);
                    }
                }, function (sender, args) { }
            );
        // Video Images List
        BrightCove.BCApp.Installer.Get_isListExists(BrightCove.BCApp.Constants.SharePointConstants.SPListID_ImageLibrary,
                function (listExists) {
                    if (listExists == false) {
                        BrightCove.BCApp.Installer.HideNavigationButtons();
                        var btnClass = $("#btnVideoImages").attr('class').replace('hidden', '');
                        $("#btnVideoImages").attr('class', btnClass);
                    }
                }, function (sender, args) { }
            );
    }
